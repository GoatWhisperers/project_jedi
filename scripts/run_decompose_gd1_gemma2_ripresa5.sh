#!/bin/bash
# run_decompose_gd1_gemma2_ripresa5.sh
#
# Ripresa post-blackout — solo i 6 concept Gd1 Gemma2 mancanti.
# Già completati: hot_vs_cold, luce_vs_buio, calma_vs_allerta
#
# Uso:
#   nohup bash scripts/run_decompose_gd1_gemma2_ripresa5.sh > /tmp/gemma2_ripresa5.log 2>&1 &
...
LOG_DIR="/tmp/decompose_gd1_gemma2_ripresa5_$(date +%Y%m%d_%H%M%S)"
...
# Solo i 6 mancanti (hot_vs_cold, luce_vs_buio, calma_vs_allerta gia OK)
CONCEPTS=(
...
check_server() {
    ...
    log "  x $name non raggiungibile ($url)"
    return 1
}
...
log "============================================================"
log "  BATCH DECOMPOSE Gd1 - Gemma2-Uncensored (ripresa5, post-blackout)"
log "  Concept: ${#CONCEPTS[@]} (skip: hot_vs_cold, luce_vs_buio, calma_vs_allerta)"
log "  Log dir: $LOG_DIR"
log "============================================================"

check_server "$STEERING_URL" "Steering server" || { log "ABORT: steering_server non risponde"; exit 1; }
check_server "$M40_URL"      "M40 llama-server" || { log "ABORT: llama-server M40 non risponde"; exit 1; }
...
while [ $waited -lt 180 ]; do
    ...
    if [ "$active" = "Gemma2-Uncensored" ]; then
        log "  v Gemma2-Uncensored caricato (${waited}s)"
        break
    fi
done
...
for CONCEPT_CAT in "${CONCEPTS[@]}"; do
    ...
    log ""
    log "  -- [${TOTAL}/${#CONCEPTS[@]}] Gemma2-Uncensored / ${CONCEPT} --"
...
log "  Totale: $TOTAL  v $SUCCEEDED  x $FAILED"
...
git diff --cached --quiet && log "  (nessuna modifica da committare)" || \
    git commit -m "data: decompose Gd1 Gemma2 ripresa5 -- ${SUCCEEDED}/${TOTAL} concept

Vettori Gd1 Gemma2-Uncensored post-blackout (6 concept mancanti).
$([ ${#FAILED_LIST[@]} -gt 0 ] && echo "Falliti: ${FAILED_LIST[*]}" || echo "Tutti OK")

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>" && \
    git push origin main && log "  v Pushato su GitHub" || log "  ! Push fallito"
